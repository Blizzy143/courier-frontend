...
function calculateDistanceToPickup(customer) {
  ...
  currentTicket.value.bill_pickup = my_path.length * 1.5;
}
...
function calculateDistanceToDelivery() {
  ...
  currentTicket.value.est_delivery_time = (pickup_blocks + my_path.length) * 3;
  currentTicket.value.est_blocks = pickup_blocks + my_path.length;
  currentTicket.value.bill_delivery = my_path.length * 1.5;
  currentTicket.value.quoted_price = (pickup_blocks + my_path.length) * 1.5;
  ...
  <v-container>
    <div id="body" v-if="currentTicket">
      ...
      <v-card class="mx-5 my-5 py-5">
        ...
        <v-card-text>
          <v-select @update:modelValue="onPickupCustomerChange"  variant="outlined" v-model="pickupcustomer" :items="customers" item-title="name" item-value="name"
            label="Pick up customer" return-object required>
          </v-select>
          <v-text-field variant="outlined" type="time" v-model="currentTicket.pickup_time" label="Pickup Time"  required></v-text-field>
